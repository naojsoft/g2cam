...
G2CAM requires Python 3.7 or higher. For some testing purposes, it may
be helpful to have {\tt astropy} installed.  
...
you may want to have an asynchronous command that validates its
parameters and then exits quickly, but which initiates continued
processing on the instrument side. This is often done for long-running
commands. For example, suppose we want a version of {\tt snap()} that
returns immediately, but initiates an exposure operation on the
instrument. Later, the instrument will finish the snap as previously
described, archive the finished fits files and then indicate
success/failure/completion of the command. Let's see how we might
accomplish that. 

\subsection{Option 1: custom dispatch method and asynchronous completion}
One option is to create your own custom dispatch method in your cam module:
\begin{verbatim}
...
\end{verbatim}
This custom dispatch command allows you to access the tag that is sent
by Gen2 to track the command completion.  This dispatch function adds
the tag to the keyword arguments, so your command methods, in addition
to accepting the normal keyword parameters, must also include a
{\tt tag} keyword. 

In the command function, you return the special value
{\tt ASYNC\_COMPLETE} that indicates that the command is asynchronous
and will be reported on later: 
\begin{verbatim}
from g2cam.Instrument import ASYNC_COMPLETE
    ...
    ...
    def snap(self, frameid=None, itime=30.0, tag=None):
        ...
        t = common.FuncTask(self._snap_async, (tag, frameid, itime))
        t.init_and_start(self)
        return ASYNC_COMPLETE

\end{verbatim}
You don't need to use a {\tt FuncTask}, of course. If you want to manage the
concurrency yourself you can initiate the {\tt snap\_async} anyway you
want. However, when the command completes you \emph{must} call
{\tt finish\_command} with the tag that was passed and the result of the
command. The result is interpreted in the same way as described above
for regular synchronous commands.
\begin{verbatim}
    ...
    ...
    def _snap_async(self, tag, frameid, itime):
        ...
        ...
        self.ocs.finish_command(tag, result)
\end{verbatim}

\subsection{Option 2: Separate commands for initiating and completing}
Some developers prefer a approach in which one command initiates the
command, and a second command is called if and when the result is
required by the observing script; this second command blocks until the
first command has completed.  In this case regular synchronous
command processing suffices. 
\begin{verbatim}
    def snap(self, frameid=None, itime=30.0):
        """Snap command that returns immediately. Command results
        are collected with command get_snap.
        """
        ...
        if not self.snap_event.is_set():
            raise CamCommandError("A snap is already running!")
        self.logger.info("Starting concurrent task")
        t = common.FuncTask(self._snap_async, (frameid, itime)
        t.init_and_start(self)

    def _snap_async(self, frameid, itime):
        self.snap_event.clear()
        self.snap_result = None
        ...
        ...
        self.snap_result = (0, "Snap completed")
        self.snap_event.set()
        
    def get_snap(self, timeout=None):
        self.snap_event.wait(timeout=timeout)

        if self.snap_result is None:
            raise CamCommandError("timed out waiting for snap result")

        return self.snap_result
\end{verbatim}
This command hands off the work to {\tt \_snap\_async()} using a
{\tt FuncTask} in another thread. 
The command {\tt get\_snap} is used to collect the result.
The synchronization between {\tt snap} and {\tt get\_snap} is managed
using the variables {\tt snap\_event} and {\tt snap\_result}.
The initialization of these variables is left as an exercise for the reader.

\subsection{Example: Sending Periodic Status}
This example is used to illustrate the sending of periodic status.
