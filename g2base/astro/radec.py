#
# radec.py -- Miscellaneous coordinate calculations
#
# B. Bon
# T. Inagaki
# E. Jeschke
...
# this can be set to achieve an earlier time
...
def degToHms(ra, precision=3):
    ...
    assert isinstance(precision, int), AstroCalcError("RA precision (%s) is not of type int" % (precision))
    assert precision >= 0, AstroCalcError("RA precision (%d) must be greater than or equal to 0" % (precision))

    sign, hours, minutes, seconds, frac = sla.dr2tf(math.radians(ra), precision)

    # dr2tf might round angles that are close to 360. deg to 360. deg,
    # which will result in hours being returned as 24. Test for that
    # case and set hours, minutes, etc., to 0.
    if hours == 24:
        hours = 0
        minutes = 0
        seconds = 0
    ...
    frac_decimal = frac / (10. ** precision)

    return (hours, minutes, seconds + frac_decimal)
...
def raDegToString(ra_deg, format='%02d%02d%06.3f', precision=3):
    # Note: make sure that the format specifier for the "seconds"
    # component has enough space for the decimal digits that will be
    # in the output string, i.e., make sure that the "format"
    # specifier is consistent with the specified
    # "precision". Otherwise, the Python string formatter will round
    # the "seconds" component and the output string will be incorrect.

    if ra_deg > 360.0:
        ra_deg = math.fmod(ra_deg, 360.0)

    hours, minutes, seconds = degToHms(ra_deg, precision)
    return raHmsToString(hours, minutes, seconds, format=format)
...
def degToDms(dec, precision=2):
    """Convert the dec, in degrees, to an (sign,D,M,S) tuple.
       D and M are integer, and sign and S are float."""

    assert dec <= 90, AstroCalcError("DEC (%f) > 90.0" % (dec))
    assert dec >= -90, AstroCalcError("DEC (%f) < -90.0" % (dec))
    assert isinstance(precision, int), AstroCalcError("Dec precision (%s) is not of type int" % (precision))
    assert precision >= 0, AstroCalcError("Dec precision (%d) must be greater than or equal to 0" % (precision))

    sign, hours, minutes, seconds, frac = sla.dr2af(math.radians(dec), precision)
    ...
    return (s, hours, minutes, seconds + frac_decimal)
...
def decDegToString(dec_deg, format='%s%02d%02d%05.2f', precision=2):
    # Note: make sure that the format specifier for the "arcseconds"
    # component has enough space for the decimal digits that will be
    # in the output string, i.e., make sure that the "format"
    # specifier is consistent with the specified
    # "precision". Otherwise, the Python string formatter will round
    # the "arcseconds" component and the output string will be
    # incorrect.
    sign, degrees, arcminutes, arcseconds = degToDms(dec_deg, precision)
    return decDmsToString(sign, degrees, arcminutes, arcseconds, format)
...
def degToTSCalpha(ra_deg):
    s = raDegToString(ra_deg)
    alpha = int('0x' + (s.replace('.', '') + '0')[:10], 16)
    return alpha

def degToTSCdelta(dec_deg):
    s = decDegToString(dec_deg)
    delta = s[1:].replace('.', '')[:8]
    if s[0] == '+':
        delta = int('0x' + delta, 16)
    else:
        delta = int('0x80' + delta, 16)
    return delta
...
def get_ra_dec_range2(ra_center,  delta_ra,
    ...
    """A more sophisticated combination of get_ra_range() and get_dec_range().
       ...
       delta_dec is the FOV in the dec direction.  Returns a tuple in the
...
def _focusCaseId( f_select, imgrot_flag ):
    ...
    """
    if  (not isinstance(f_select, str) or
        f_select.lower() not in _stMapping.keys()):
        # ERROR -- can't determine focus!
        return '_focusCaseId: illegal f_select value %s' % repr(f_select)
    ...
    if  st == 2 or st == 4 or st == 7:
        if  imgrot_flag != 0 and imgrot_flag != 1:
            ...
            return '_focusCaseId: illegal imgrot_flag value %s' % repr(imgrot_flag)
        elif  imgrot_flag == 1:
            st += 1
    return st
...
def _gdofst( st, p, th, da, dd, a0, h0, sv=False, logger=None):
    """
    Offset on the guider (from gdofst.c)
...
def calc_Getoffset( f_select, az, el, ra_rel, dec_rel, origin_x1, origin_y1,
    ...
    nreadout_x1 = new_readout[0]
    nreadout_y1 = new_readout[1]
    nreadout_x2 = new_readout[2]
    nreadout_y2 = new_readout[3]
    if logger is not None:
...
def epoch():
    ...
    """
    # global_time_offset is normally 0, but can be set to achieve a kind
    # of "system time" for Gen2 that is offset from the present
    val = time.time() - global_time_offset
    return val

def prcsn1(oe,ne,ra,de):
    """
    Precession calculation
    Copyright reserved by Wataru Tanaka
...
def _gdeqrt(st,jd,eo,es,pr,r0,d0,rs,ds,cx,cy):
    ...
    Copyright reserved by Wataru Tanaka (1998) (gdeqrt.c)
...
def delta_pa_stars(ra0, dec0, ra1, dec1):
    """
    ...
    (used for star selection)
    """
    # if star is the same postion as the zenith
    swap = False
    if fabs(dec0) == half_pi:
        ra0, ra1, dec0, dec1 = ra1, ra0, dec1, dec0
        swap = True
    ...
    # reduce the number of cos/sin calculations
    cosddec1 = cos(ddec1)
    cosddec0 = cos(ddec0)
    sinddec0 = sin(ddec0)
    sinddec1 = sin(ddec1)
    ...
    try:
        delta = acos(cosddec0*cosddec1 + sinddec0*sinddec1*cos(dra)) # distance
    except Exception:
        # !!???
        pa = delta = 0 #

    # pa is 0 if delta is 0
    if not delta == 0:
        sindelta = sin(delta)

        y = sinddec1 * sin(dra) / sindelta
        x = (cosddec1-cosddec0*cos(delta)) / (sinddec0*sindelta)
...
def delta_stars(ra0, dec0, ra1, dec1):
    ...
    try:
        # distance
        delta = acos(cos(ddec0)*cos(ddec1) + sin(ddec0)*sin(ddec1)*cos(dra))
...
def pa_stars(ra0, dec0, ra1, dec1, delta=None):
    ...
    y = sin(ddec1) * sin(dra) / sin(delta)
    x = (cos(ddec1)-cos(ddec0)*cos(delta)) / (sin(ddec0)*sin(delta))
...
def radec_deg_abs_to_azel_deg(ra_deg, dec_deg, localtime_t=None,
    ...
    """Convert RA/DEC to an (AZ, EL) tuple (in degrees).
    ra_deg: RA degrees (float)
    dec_deg: DEC degrees (float)
    localtime_t: seconds since the epoch in local time; if None
       use the current time
    ra_off_deg: RA offset degrees (float, if not None)
    dec_off_deg: DEC offset degrees (float, if not None)
    units: 'degrees' (default) or other (will return radians)
    ...
    if ra_off_deg:
        ra_off_rad = math.radians(ra_off_deg)
        ra_rad += ra_off_rad
    if dec_off_deg:
...
def calcAzEl(ra, dec, localtime_t=None, ra_off=None, dec_off=None,
    ...
    ra_deg = funkyHMStoDeg(ra)
    dec_deg = funkyDMStoDeg(dec)

    # Calculate offsets, if supplied
    ra_off_deg = 0.0
    if ra_off:
        ra_off_deg = funkyHMStoDeg(ra_off)
    dec_off_deg = 0.0
    if dec_off:
        dec_off_deg = funkyDMStoDeg(dec_off)
    ...
    return radec_deg_abs_to_azel_deg(ra_deg, dec_deg,
                                     ...
                                     ra_off_deg=ra_off_deg,
                                     dec_off_deg=dec_off_deg,
                                     logger=logger)

def calc_AzEl(ra, dec, localtime_t=None, ra_off=None, dec_off=None,
              logger=None):
    """
    Just another way to do the above function, but using a different
    way to handle the offsets.
    """

    # Calculate offsets, if supplied
    ra_offset = dec_offset = 0
    if ra_off is not None:
        ra_offset = ra_off
    if dec_off is not None:
        dec_offset = dec_off
    ra, dec = calc_RaDec(1, ra, dec, ra_offset, dec_offset, logger=logger)
...
def calc_best_az(cmd_az_deg, cur_az_deg, min_az, max_az):
    # TODO: calculate the fastest (or best) azimuth for the commanded
    # azimuth (cmd_az_deg) based on the current azimuth (cur_az_deg)
    if cmd_az_deg < 0:
        az_neg, az_pos = cmd_az_deg, math.fmod(cmd_az_deg + 360.0, 360.0)

    else:
        az_neg, az_pos = math.fmod(cmd_az_deg - 360.0, 360.0), cmd_az_deg

    diff_neg = math.fabs(cur_az_deg - az_neg)
    diff_pos = math.fabs(cur_az_deg - az_pos)
